...
static void createTable(QSqlDatabase db) {
  ...
  if (db.open()) {
    ...
    if (!query.exec("CREATE TABLE IF NOT EXISTS USERINFO ("
                    ...
                    "ICON TEXT DEFAULT "
                    "'https://c-ssl.duitang.com/uploads/blog/201408/15/"
                    "20140815095903_ttcnF.jpeg',"
                    "GENDER BOOLEAN,"
                    "BIRTH TEXT,"
                    "AREA CHAR(20),"
                    "EDUCATION CHAR(20),"
...
SqlAccountModel::~SqlAccountModel() { database().close(); }

QByteArray SqlAccountModel::addUserAccount(const QString &userName,
...
QByteArray SqlAccountModel::checkAccount(const int &userID,
                                         const QString &userPassword) {
  bool rel;
  QString password;
  int id;
...
void SqlAccountModel::updateUserInfo(const int &userID, const QString &name,
                                     const int &gender, const QString &birth,
                                     const QString &area,
...
QByteArray SqlAccountModel::messageList(const int &ID) {
  ...
  if (!query.exec(QString("SELECT * FROM FRIENDMESSAGEINFO "
    ...
    while (query.next()) {
      ...
      if (lastID == 0 || lastID == query.value("ID").toInt()) {
        ...
        set.remove(ID);
        jsonTotalF.insert(QString::number(*(set.begin())),
                          QJsonValue(jsonItem));
        lastID = query.value("ID").toInt();
        set.clear();
        set.insert(query.value("SENDID").toInt());
        set.insert(query.value("RECEIVEID").toInt());
        while (!jsonItem.empty())
          jsonItem.removeLast();
        obj.insert("sid", QJsonValue(query.value("SENDID").toInt()));
        obj.insert("rid", QJsonValue(query.value("RECEIVEID").toInt()));
        obj.insert("datetime", QJsonValue(query.value("DATETIME").toInt()));
  ...
  while (!jsonItem.empty())
    jsonItem.removeLast();
  if (!query.exec(
          QString("SELECT GROUPMESSAGEINFO.GID AS GID, GROUPMESSAGEINFO.MID AS "
                  "MID, GROUPMESSAGEINFO.DATETIME AS DATETIME, "
                  "GROUPMESSAGEINFO.MESSAGE AS MESSAGE "
                  "FROM MEMBERINFO join GROUPMESSAGEINFO ON "
                  "MEMBERINFO.GID=GROUPMESSAGEINFO.GID "
                  "WHERE MEMBERINFO.MID=%1 ORDER BY GID ASC, DATETIME ASC")
