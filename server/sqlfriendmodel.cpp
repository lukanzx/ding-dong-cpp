    ...
    if (db.open()) {
      ...
      if (!query.exec(
              QString("CREATE TABLE IF NOT EXISTS %1("
                      ...
                      "FOREIGN KEY (USERID) REFERENCES USERINFO(ID) ON DELETE "
                      "CASCADE,"
                      "FOREIGN KEY (FRIENDID) REFERENCES USERINFO(ID) ON "
                      "DELETE CASCADE)")
                  .arg(friendTableName))) {
        qDebug() << "表创建发生错误";
...
SqlFriendModel::~SqlFriendModel() { database().close(); }
...
QByteArray SqlFriendModel::friendList(const int &ID) {
  ...
  if (!query.exec(
          QString("SELECT * FROM (SELECT USERID AS ID FROM FRIENDINFO WHERE "
...
void SqlFriendModel::sendMessage(const int &sendID, const int &receiveID,
                                 const int &type, const int &datetime,
                                 const QString &message) {
  QSqlQuery query;
  int id;
  if (!query.exec(
          QString("SELECT ID FROM FRIENDINFO WHERE "
                  "(USERID=%1 AND FRIENDID=%2) OR (USERID=%2 AND FRIENDID=%1)")
              .arg(QString::number(sendID), QString::number(receiveID)))) {
    qDebug() << "选择好友对编号发生错误";
    qDebug() << query.lastError();
  }
  query.next();
  ...
  if (!query.exec(QString("INSERT INTO FRIENDMESSAGEINFO VALUES("
                          "%1, %2, %3, %4, '%5', %6)")
                      .arg(QString::number(id), QString::number(sendID),
                           QString::number(receiveID),
                           QString::number(datetime), message,
                           QString::number(type)))) {
    qDebug() << "添加好友聊天信息发生错误";
    qDebug() << query.lastError();
  } else {
...
QByteArray SqlFriendModel::messageList(const int &aID, const int &bID) {
  ...
  if (!query.exec(QString("SELECT * FROM FRIENDMESSAGEINFO "
                      ...
                      .arg(QString::number(aID), QString::number(bID)))) {
    qDebug() << "选择好友聊天信息错误";
    qDebug() << query.lastError();
  } else {
    qDebug() << "选择好友聊天信息成功";
