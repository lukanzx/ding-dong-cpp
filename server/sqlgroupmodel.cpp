  ...
  if (!db.tables().contains(groupTableName)) {
    if (db.open()) {
      ...
      if (!query.exec(QString("CREATE TABLE IF NOT EXISTS %1("
                              ...
                              "ICON TEXT DEFAULT "
                              "'https://c-ssl.duitang.com/uploads/blog/201408/"
                              "15/20140815095903_ttcnF.jpeg',"
    ...
    if (db.isOpen()) {
      ...
      if (!query.exec(
              QString("CREATE TABLE IF NOT EXISTS %1("
                      ...
                      "MID INTEGER NOT NULL REFERENCES USERINFO(ID) ON DELETE "
                      "CASCADE,"
...
SqlGroupModel::~SqlGroupModel() { database().close(); }
...
int SqlGroupModel::createGroup(const int &masterID) {
  ...
  if (!query.exec(QString("INSERT INTO GROUPINFO(NAME) VALUES("
                          "'%1')")
                      .arg(QString::number(masterID) + "s Group"))) {
    qDebug() << "创建群聊发生错误";
    qDebug() << query.lastError();
  } else {
    qDebug() << "创建群聊成功";
...
void SqlGroupModel::updateGroupInfo(const int &id, const QString &name,
                                    const QString &intro,
...
bool SqlGroupModel::joinGroup(const int &gID, const int &mID, const int &rank) {
  ...
  if (!query.exec(QString("INSERT INTO MEMBERINFO(GID, MID, RANK) VALUES("
                          "%1, %2, %3)")
                      .arg(QString::number(gID), QString::number(mID),
                           QString::number(rank)))) {
    qDebug() << "加入群聊发生错误";
    qDebug() << query.lastError();
    return false;
...
void SqlGroupModel::sendMessage(const int &gID, const int &mID, const int &type,
...
QByteArray SqlGroupModel::groupList(const int &mID) {
  ...
  QJsonObject finalObj;
  QJsonArray jsonAry;
  if (!query.exec(QString("SELECT ID, NAME, ICON, INTRO, NOTICE FROM "
                          "MEMBERINFO JOIN GROUPINFO "
...
QByteArray SqlGroupModel::messageList(const int &gID) {
  ...
  if (!query.exec(QString("SELECT * FROM GROUPMESSAGEINFO "
                          "WHERE GID=%1 ORDER BY GID ASC, DATETIME ASC")
                      .arg(QString::number(gID)))) {
    qDebug() << "选择群聊天信息错误";
    qDebug() << query.lastError();
  } else {
